...
def done_rows(conn:sqlite3.Connection)->list[sqlite3.Row]:return conn.execute("SELECT doc_id, name, folder, filename, relpath, content_type FROM documents WHERE status = 'done'").fetchall()
def set_saved(conn:sqlite3.Connection,doc_id:str,filename:str,relpath:str)->None:conn.execute('UPDATE documents SET filename = ?, relpath = ?, updated_at = ? WHERE doc_id = ?',(filename,relpath,_now(),doc_id));conn.commit()
